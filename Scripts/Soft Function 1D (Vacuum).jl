...
for (i, g) in enumerate(g_list)
    ...
    SF_data = h5open("SF_data/N=$N,g=$g,m=$mass,dp=$dp,vacuum", "w")
    SF_MPS = h5open("SF_MPS/N=$N,g=$g,m=$mass,dp=$dp,vacuum", "w")
    for t in t_range
...
S_t_list_g = Dict()
for g in g_list
    SF_data = h5open("SF_data/N=$N,g=$g,m=$mass,dp=$dp,vacuum", "r")
